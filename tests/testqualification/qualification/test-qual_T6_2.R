...
testthat::test_that("Adverse Event Assessments can be done correctly using a grouping variable, such as Site or Country for KRIs, and Study for QTLs, when applicable.", {
  ...
  iwalk(test, ~ expect_equal(
    n_distinct(.x$Mapped_SUBJ[[kri_workflows[[.y]]$steps[[which(map_chr(kri_workflows[[.y]]$steps, ~ .x$name) == "gsm.core::Input_Rate")]]$params$strGroupCol]]),
  ...
  kri_custom2 <- map(kri_workflows, function(kri) {
    kri$steps[[which(map_chr(kri$steps, ~ .x$name) == "gsm.core::Input_Rate")]]$params$strGroupCol <- "agerep"
    kri$steps[[which(map_chr(kri$steps, ~ .x$name) == "gsm.core::Input_Rate")]]$params$strGroupLevel <- "Age"
    return(kri)
  })
  ...
  iwalk(test_custom2, ~ expect_identical(
    sort(unique(.x$Analysis_Input$GroupID)),
    sort(unique(.x$Mapped_SUBJ[[kri_custom2[[.y]]$steps[[which(map_chr(kri_workflows[[.y]]$steps, ~ .x$name) == "gsm.core::Input_Rate")]]$params$strGroupCol]]))
  ))
