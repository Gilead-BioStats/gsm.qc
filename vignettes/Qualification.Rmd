...
subtitle: "`r paste0('{gsm.core} v', packageVersion('gsm.core'), ' ',
              '{gsm.mapping} v', packageVersion('gsm.mapping'), ' ',
              '{gsm.kri} v', packageVersion('gsm.kri'), ' ',
              '{gsm.reporting} v', packageVersion('gsm.reporting'))`"
...
```{r prs, eval = FALSE}
release <- gh("/repos/Gilead-BioStats/gsm.qc/releases", .token = remotes:::github_pat())[[1]]
...
while(pr_at_date != TRUE){
  resp <- gh("/repos/Gilead-BioStats/gsm.qc/pulls", per_page = 100, .token = remotes:::github_pat(),
...
getRepoDetails <- function(x) {
  ...
  tempReviews <- gh(paste0("GET /repos/Gilead-BioStats/gsm.qc/pulls/", PullRequest, "/reviews"))
